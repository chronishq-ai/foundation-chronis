...
The Phase 2 work is applied as targeted hardening on top of the existing implementation. The original sprint architecture is preserved; no new phase-level folder is introduced.
...
## Core Cold Start & Threshold Calibration
...
## Auxiliary Intelligence Modules
...
The Phase 1 implementations are kept intact. Phase 2 remains a separate
hardening layer, so the original Phase 1 and Phase 2 source files are not
merged or rewritten. Two small package markers were added only where needed
so the Phase 2 test suites can import their local Phase 2 modules while still
resolving shared Phase 1 modules; no existing Sprint 10 or Sprint 12 source
file was modified.
...
The Sprint 10 suites currently pass:
...
Phase 2 has `124/124` tests passing, including the strengthened specificity
and citation contracts. **Local regression:** `124 passed`.

### Reproduction commands

Run each phase from its own directory. Phase 2 uses the matching Phase 1
folder for shared upstream interfaces/modules:

```bash
# Sprint 10 Phase 1
cd "sprint10/sprint10 phase1"
PYTHONPATH=. python -m pytest -q

# Sprint 10 Phase 2
cd "../sprint10 phase2"
PYTHONPATH="../sprint10 phase1" python -m pytest -q
...
# Sprint 12 Phase 1
cd "../sprint12/sprint12 phase1"
PYTHONPATH=. python -m pytest -q

# Sprint 12 Phase 2
cd "../sprint12 phase2"
PYTHONPATH="../sprint12 phase1" python -m pytest -q
```
...
Sprint 10 and Sprint 12 contain their Phase 1 implementation with the reviewed Phase 2 hardening consolidated into the same working tree. Sprint 11 contains Kuheli's reviewed implementation. There are no `sprint10 phase1`, `sprint10 phase2`, `sprint12 phase1`, or `sprint12 phase2` directories in this delivery.
